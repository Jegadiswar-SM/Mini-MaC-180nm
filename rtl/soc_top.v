module soc_top (
    ...
    // -----------------------------------------------------------------
    // Dedicated Local SRAM buffers feeding the 12x12 array boundary
    // -----------------------------------------------------------------
    ...
    sram_wrapper #(.DEPTH(1024), .ADDR_WIDTH(10)) u_buf_wgt (
        .clk(clk),
        .sram_cen(sram_wgt_cen),
        .sram_wen(!dma_m_we),
        .sram_addr(dma_m_addr[11:2]),
        .sram_wmask(4'hF),
        .sram_din(dma_m_wdata),
        .sram_dout(sram_wgt_dout)
    );

    sram_wrapper #(.DEPTH(512), .ADDR_WIDTH(9)) u_buf_act (
        .clk(clk),
        .sram_cen(sram_act_cen),
        .sram_wen(!dma_m_we),
        .sram_addr(dma_m_addr[10:2]),
        .sram_wmask(4'hF),
        .sram_din(dma_m_wdata),
        .sram_dout(sram_act_dout)
    );

    sram_wrapper #(.DEPTH(512), .ADDR_WIDTH(9)) u_buf_res (
        .clk(clk),
    ...
    axi_stream_dma u_mac_dma (
        ...
        .dma_len_wgt_i(32'd144),
        .dma_len_act_i(32'd144),
        .busy_o(),
        .wgt_done_o(),
        .act_done_o(),
        .res_done_o(),
    ...
    mac_multicore #(
        .ROWS(12), .COLS(12), .DATA_W(8), .NUM_CORES(4)
    ) u_mac (
        .clk(clk), .rst_n(rst_n_int),
    ...
    assign mac_global_start = u_mac.u_regs.global_start_o;
    assign mac_gnt_vec      = {mac3_gnt, mac2_gnt, mac1_gnt, mac0_gnt};
